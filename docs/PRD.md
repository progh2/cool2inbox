...
| FR-1.3 | 폴더 자동 탐지 — `LOCALAPPDATA`/`APPDATA`/`PROGRAMDATA`/`USERPROFILE` 아래 관례 경로와 계정별 하위 폴더를 훑어 `.udb` 가 있는 첫 폴더를 고른다. 실패 시 사용자가 직접 지정 |
| FR-1.4 | 폴더에 `.udb` 가 여럿이면 **가장 최근 수정된 것**을 쓴다 (구버전 파일 공존 대비) |
...
| FR-2.3 | 수신 파일 폴더는 자동 탐지 후 설정에서 변경 가능 (기본 후보: `%USERPROFILE%\Documents\CoolMessenger\`, `...\받은 파일\`, 쿨메신저 설정값 — **R1에서 함께 확정**) |
...
| **R2** | 쿨메신저 **수신 파일 저장 폴더**의 실제 위치·구조 미확인 | 첨부 복사 불가 | R1과 함께 확인. 쿨메신저 설정 화면의 경로도 참고 |
| R3 | 쿨메신저 버전 업데이트로 스키마 변경 | 동작 중단 | 스키마 검증 후 명확한 안내 + 필수/선택 컬럼 분리로 부분 동작 유지 |
| R4 | 첨부 원본이 이미 삭제됨(오래된 쪽지 백필 시 흔함) | 첨부 누락 | 파일명은 md에 남기고 "원본 없음" 표기. 실패가 아님 |
