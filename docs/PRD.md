...
| FR-2.7 | 첨부 원본이 아직 없거나 복사에 실패해도 **쪽지 저장 실패로 취급하지 않는다.** md는 저장하고 '원본 없음'을 남긴 뒤 **매 폴링마다 다시 찾는다.** 쿨메신저는 사용자가 첨부를 열기 전까지 파일을 PC에 내려받지 않으므로(실물에서 첨부 278건이 전부 서버 세션ID 보유, 디스크엔 5개뿐), 나중에 Received Files에 생기면 자동으로 md에 링크가 붙는다. md 재작성은 우리가 쓴 그대로일 때만 하고, 사용자가 편집했으면 파일만 복사한다 |
...
| D12 | 보낸 쪽지 지원 (설정으로 on/off, 기본 꺼짐) | 사용자 요청으로 편입. `tbl_send` 는 `tbl_recv` 와 거의 동일(Receiver/SendDate). **보낸쪽지 하위 폴더로 분리** 저장, 중복 이력은 (kind, key) 복합 키로 받은 쪽지와 분리 |
| D13 | 첨부 원본은 **복사**한다 (이동·삭제 안 함) | 사용자 선택(2026-09-02). 원본 무손상 원칙 유지. Received Files 정리는 사용자 몫으로 둔다 |
| D14 | 쿨메신저 **파일 서버에서 직접 내려받지 않는다** | 포트 46001 은 열려 있으나 HTTP 가 아닌 **암호화된 독자 프로토콜**이다(연결 즉시 바이너리 헤더, 응답이 매번 랜덤 바이트로 확인됨). 리버스 엔지니어링 + 계정 인증이 필요해 '읽기 전용' 원칙과 유지보수성에 모두 어긋난다. 대신 첨부 재시도(FR-2.7)로 같은 편의를 안전하게 제공한다 |
